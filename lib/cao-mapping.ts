...
export function getCao(
  ...
  if (["oosterpoort", "klinq", "timon", "youke", "youké", "kinabu", "nabucco", "ivg kind", "veilig thuis", "pluryn"].some((x) => o.includes(x))) return "jeugd";
  if (["woondroomzorg", "busehoek", "eigen thuis", "toekomst"].some((x) => o.includes(x))) return "vvt";
  if (["smo", "traverse", "tussenvoorziening", "het werkt"].some((x) => o.includes(x))) return "sw";
